...
model = ConvNet(3, 16, 3, 2, 1)
model = torch.compile(model.to('cuda'), backend="inductor")

try:
    x = torch.randn(32, 3, 32, 32, device="cuda", dtype=torch.float32)
    ...
    with open('results_convolution_1.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([32, 3, 16, 3, 2, 32, 32, flops, ms, "external"])
except Exception as e:
    error_msg = "There was an exception running the following parameters: 32, 3, 16, 3, 2, 32, 32, external on gpu:1\n"

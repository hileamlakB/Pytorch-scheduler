...
image_sizes = [32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768,65536,131072]
...
with open('results_convolution.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["In Channels", "Out Channels", "Kernel Size", "Stride", "Input Size", "Flops", "Latency", "Latency Type"])


for ltype in ["intenal", "external"]:
    for i, (in_channels, out_channels, kernel_size, stride, image_size) in enumerate(params):
        ...
        if image_size**2 * 4 * in_channels > max_memory:
            print("Skipping, image size too large, ", image_size**2 * 4 * in_channels)
            continue
    ...
    with open('results_convolution.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([{in_channels}, {out_channels}, {kernel_size}, {stride}, {image_size}, status['flops'], status['ms'], {ltype}])
    """)
